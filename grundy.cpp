//./a.out > graph.dot ; dot -Tpng graph.dot -o graph.png

// Utiliser set comme structure de graph pour eviter le multigraphe
...
typedef adjacency_list<vecS, vecS, bidirectionalS, int, setS> Graph;
...
void random_graph(Graph& g, int nb_vertex, int perc) {
    ...
    std::uniform_int_distribution<unsigned long> dis2(1, (nb_vertex * perc / 100)+1);
